/**
 * Benchmarks warm `generate()` cost per query against a realistic-sized schema.
 * Run on base and PR on the same runner (absolute timings vary ~3x across
 * runners), so only the relative delta is meaningful. Writes JSON to argv[2] or
 * stdout. Two metrics:
 *   - distinct: every query text unique -> the uncached generate path (the gate).
 *   - repeated: a fixed set re-linted -> exercises the result cache.
 */
...
// Real apps have many tables; the test migrations alone (~15) understate the
// per-query cost, so pad to a realistic size.
...
async function measure(decorate: Decorate) {
  ...
  const gen = (text: string) =>
    generator.generate({ sql, query: { text, sourcemaps: [] }, cacheKey: DB_URL, fieldTransform: undefined });
...
async function main() {
  ...
  // distinct: round + index make every text unique -> cache always misses.
  const distinct = await measure((text, round, index) => `${text} /*d${round}-${index}*/`);
  // repeated: a fixed set re-linted every round -> cache hits after warm-up.
