...
Nothing about Kysely, Drizzle, Prisma, or TypeORM is baked into the core. This was
proven twice while building Kysely support:

- **Embedded-sql validation** (`resolveQuery`) compiles a builder chain — through the raw
  `sql` fragments embedded in it — to SQL **inside the Kysely plugin** (its own `DummyDriver`
  sandbox) and hands SafeQL only the SQL text.
- **Schema-drift validation** (`check-schema`) added an entirely new validation mode
  via the `resolveSchemaType` hook + an `introspectSchema` worker endpoint **without a
  single change to the core query seam**.
...
| `resolveSchemaType` | rule (checker) | Locate + interpret a library schema type (e.g. a Kysely `Database` interface) for `check-schema`. |
...
Rule-only hooks (`onTarget`, `onExpression`, `resolveQuery`, `resolveSchemaType`) run in
...
same shape as Drizzle/Kysely `sql` tags. A Prisma plugin implements `onTarget`
(recognise the `$queryRaw`/`$executeRaw` tag) and `onExpression` (translate the `Prisma.*`
...
takes a plain SQL **string** rather than a tag, so query validation would use
`resolveQuery` over a `CallExpression` (à la the builder pre-filter) rather than
`onTarget`.
